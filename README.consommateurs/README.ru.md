...
graphify {
    ...
    // Опциональный инкрементальный пересекан (по умолчанию: false → полный скан).
    incremental.set(true)

    // Где хранится кэш отпечатков (по умолчанию: build/graphify/fingerprints.json).
...
`collectFromWorkspace` сканирует всё дерево в обоих режимах; именно
обход обнаруживает добавленные и удалённые файлы. При `incremental.set(true)`
*разбор каждого файла* мемоизируется:

- файл, размер и mtime которого не изменились, переиспользует извлечение из кэша (без чтения);
- файл, у которого изменился mtime, но содержимое осталось прежним, определяется по SHA-256
  и переиспользует извлечение идентичного файла (например, после `git checkout`);
- заново разбирается только действительно новое содержимое.

Кэш — это документ JSON в каталоге `build/` (никогда не коммитится), и он усекается до
файлов, присутствующих при каждом сканировании. Это лишь оптимизация: отсутствующий,
повреждённый или неизвестной версии кэш молча деградирует до полного скана, а полученный
`graph.json` побайтово идентичен неинкрементальному запуску.
