...
graphify {
    ...
    // অপ্ট-ইন ইনক্রিমেন্টাল পুনঃস্ক্যান (ডিফল্ট: false → সম্পূর্ণ স্ক্যান)।
    incremental.set(true)

    // ফিঙ্গারপ্রিন্ট ক্যাশে কোথায় সংরক্ষিত হয় (ডিফল্ট: build/graphify/fingerprints.json)।
    cacheFile.set(layout.buildDirectory.file("graphify/fingerprints.json").get().asFile)
}
```

## ইনক্রিমেন্টাল স্ক্যান

`collectFromWorkspace` উভয় মোডেই সম্পূর্ণ ট্রি স্ক্যান করে; হাঁটা/পরিভ্রমণই
যোগ ও বাদ দেওয়া ফাইল শনাক্ত করে। `incremental.set(true)` থাকলে, *প্রতি-ফাইল
পার্সিং* মেমোাইজ করা হয়:

- যে ফাইলের আকার ও mtime অপরিবর্তিত, তা তার ক্যাশ করা এক্সট্র্যাকশন পুনঃব্যবহার করে (কোনো পড়া নেই);
- যে ফাইলের mtime বদলেছে কিন্তু বিষয়বস্তু অপরিবর্তিত, তা SHA-256 দিয়ে শনাক্ত করা হয়
  এবং একটি অভিন্ন ফাইলের এক্সট্র্যাকশন পুনঃব্যবহার করে (যেমন `git checkout`-এর পরে);
- কেবল সত্যিই নতুন বিষয়বস্তুই পুনঃপার্স করা হয়।

ক্যাশেটি `build/`-এর অধীনে একটি JSON ডকুমেন্ট (কখনো কমিট করা হয় না) এবং প্রতিটি
স্ক্যানে উপস্থিত ফাইলগুলিতে ছেঁটে আনা হয়। এটি কেবল একটি অপটিমাইজেশন: অনুপস্থিত, নষ্ট
বা অজানা-সংস্করণের ক্যাশে নীরবে একটি সম্পূর্ণ স্ক্যানে অবনমিত হয়, এবং উৎপন্ন
`graph.json` একটি অ-ইনক্রিমেন্টাল রানের সাথে বাইট-অভিন্ন থাকে।
